...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from ThinkingBridge Github (http://github.com/ThinkingBridge)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/ThinkingBridge/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'ThinkingBridge/%s already exists' % (repo_name)
            else:
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for ThinkingBridge/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: ThinkingBridge/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "ThinkingBridge/%s" % repo_name, "revision": "kitkat" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/thinkingbridge.dependencies'
    syncable_repos = []
...
print "Repository for %s not found in the ThinkingBridge Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/roomservice.xml" % device
